...
app.get('/api/businesses',(req,res)=>{
 ...
 let sql=`SELECT * FROM businesses WHERE status='approved' AND city=?`,args=[city];if(top){sql+=' AND top=?';args.push(top)}if(sub){sql+=' AND sub=?';args.push(sub)}
 const rows=db.prepare(sql+' ORDER BY updated_at DESC LIMIT 1000').all(...args).filter(r=>{const text=norm([r.name,r.area,r.address,r.top,r.sub].join(' '));return(!q||text.includes(q))&&(!area||norm(r.area).includes(area)||norm(r.address).includes(area))}).slice(0,limit);res.json({items:rows.map(publicRow),count:rows.length});
});
